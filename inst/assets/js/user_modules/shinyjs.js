// Already refactored
export function toggle(id, anim = false, animType = 'slide', time = 0.5) {
  console.log(`shinyjs.toggle(id = ${id}, anim = ${anim}, animType = ${animType}, time = ${time})`)
  var obj = $(`#${id}`)
...
function show(id = null, type = 'slide', time=0.5) {
  // id: e.g. "myID"
  // type: either "none", "fadeIn", "slide"
  console.log(`shinyjs.show(id = ${id}, type = ${type}, time = ${time})`)
  var obj = $(`#${id}`)
  if (anim) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// TO BE REFACTORED
// Copied from https://github.com/daattali/shinyjs/blob/master/inst/srcjs/shinyjs-default-funcs.js
// Should only be kept for reference!
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function _getElements (params) {
  ...
  if (params.elements !== null && typeof params.elements !== 'undefined') {
    ...
    typeof params.selector !== 'undefined'
  ) {
    $els = $(params.selector)
  }
  if ($els === null || $els === undefined || $els.length == 0) {
    debugMessage('shinyjs: Could not find DOM element using these parameters:')
    debugMessage(params)
    $els = null
  }
  return $els
...
function _initDisabledHelper (els) {
...
function _showHide (method, params) {
  var defaultParams = {
    id: null,
    anim: false,
    animType: 'slide',
...
function _addRemoveClass (method, params) {
  var defaultParams = {
    id: null,
    class: null,
    selector: null,
  ...
  method == 'add' ? $els.addClass(params.class) : $els.removeClass(params.class)
}

// ---------------------------------------------------
// ------ Helpers for `enable`/`disable` -------------

function _enableDisable (method, params) {
  var defaultParams = {
    id: null,
    selector: null,
    elements: null
  ...
  $.map($els, function (el) {
    ...
    var toadd = $el.find(
      'input, button, textarea, select, a[download], span.btn.btn-file'
    )
...
function _initResettablesHelper (els) {
  for (var j = 0; j < els.length; j++) {
    ...
    // dateRangeInput
      ...
      inputType = 'DateRange'
      inputValue =
        _getInputDate($(input.find('input')[0])) +
        ',' +
    ...
    if (foundInput) {
      ...
      input
        .attr('data-shinyjs-resettable-id', inputId)
...
function _oneventAttach (params) {
...
export function debugMessage (text) {
  if (debug) {
    console.info(text)
  }
}
...
// so that users can define their own `init` function (using extendShinyjs)
...
export function toggleClass (params) {
  var defaultParams = {
    id: null,
    class: null,
    condition: null,
  ...
  if (params.condition === null) {
    ...
    $.map($els, function (el) {
      params.elements = $(el)
      $(el).hasClass(params.class) ? removeClass(params) : addClass(params)
    })
...
export function toggleState (params) {
  var defaultParams = {
    id: null,
    condition: null,
    selector: null
  ...
  if (params.condition === null) {
    ...
    $.map($els, function (el) {
      params.elements = $(el)
      _isDisabled($(el)) ? enable(params) : disable(params)
    })
  } else if (params.condition) {
    enable(params)
  } else {
...
export function onevent (params) {
  var defaultParams = {
    event: null,
    id: null,
    shinyInputId: null,
