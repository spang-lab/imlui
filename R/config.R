...
# Return path to cache directory (<home>/.imlui/cache).
# TODO: XDG compliance.
get_imlui_cache_dir <- function() {
  trace_func_entry()
  cacheDir <- file.path(toscutil::home(), ".imlui", "cache")
...
gsub_yml <- function(yml, IMLUI_CONFIG_DIR) {
  trace_func_entry("gsub_yml")
  x <- if (is.list(yml)) {
    lapply(yml, gsub_yml, IMLUI_CONFIG_DIR)
  } else {
    gsub(
